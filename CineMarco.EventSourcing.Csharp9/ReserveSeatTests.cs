...
namespace CineMarco.EventSourcing.Csharp9
{
    public class ReserveSeatTests : TestBase
    {
        ...
        {
            Given(
                new ScreeningInitialized(screening1, new NumberOfSeats(10))
            );

            When(
                new ReserveSeats(screening1, new NumberOfSeats(2))
            );

            ThenExpect(
                new SeatsReserved(screening1, new NumberOfSeats(2))
            );
        }
        ...
        public void Reserve_Too_Much_Seats_Should_Fail()
        {
            ...
            When(
                new ReserveSeats(screening1, new NumberOfSeats(20))
            );

            ThenExpect(
                new SeatsNotReserved(screening1, new NumberOfSeats(20))
            );
        }
        ...
        public void Reserve_Should_Fail_Given_All_seats_are_reserved()
        {
            ...
            When(
                new ReserveSeats(screening1, new NumberOfSeats(1))
            );

            ThenExpect(
                new SeatsNotReserved(screening1, new NumberOfSeats(1))
            );
        }
    }
}